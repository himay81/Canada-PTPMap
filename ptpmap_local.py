...
def styleLink(licName, kmlLink):
    ...
    if licNameLow.find('bell') != -1:               kmlLink.style = bellStyle
    elif licNameLow.find('rogers') != -1:           kmlLink.style = rogersStyle
    elif licNameLow.find('telus') != -1:            kmlLink.style = telusStyle
    elif licNameLow.find('xplornet') != -1:         kmlLink.style = xplornetStyle
    elif licNameLow.find('freedom mobile') != -1:   kmlLink.style = freedomStyle
    else:                                           kmlLink.style = otherStyle
...
if __name__ == "__main__":
    ...
    csvstart = timer()
    csvd = pd.read_csv('TAFL_LTAF.csv', names=names.keys(),
                       usecols=[name for name, toimport in names.items() if toimport])
    csvstop = timer()
